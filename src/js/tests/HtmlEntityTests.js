		...
		test: function HTML_entity_tests() {
			return [
				...
				function Should_require_value_property() {
					Assert.willThrow(new ModuleException("The \"value\" property must be set to a valid HTML entity"));
					parser.parse("[:entity]");
				},
				...
				function Should_validate_entity_value() {
					Assert.willThrow(new ModuleException("\"<script>malicious code</script>\" is not a valid HTML entity"));
